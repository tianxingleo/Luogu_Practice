...
int main(){
    ...
    for(int i=0;i<=M-2;i++){
        if(destination[i+1]-destination[i]<0){
            diff[destination[i]-1-1+1]--;diff[destination[i+1]-1]++;
        }else if(destination[i+1]-destination[i]>0){
            diff[destination[i]-1]++;diff[destination[i+1]-2+1]--;
        }
        
    }

    //复原
    destination_sum[0]=diff[0];
    for(int i=1;i<=N-2;i++){
        destination_sum[i] = destination_sum[i-1] + diff[i];
    }
